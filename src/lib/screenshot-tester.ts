...
import { TestResult, ViewportConfig } from '@shared/index.js';
...
export class ScreenshotTester {
  ...
  async captureAboveTheFoldScreenshot(
    page: Page,
    pageUrl: string,
    viewport: ViewportConfig,
    sessionId: string
    ...
    // Create initial test result with viewport-specific test type
    const testResult = this.sessionManager.createTestResult(`screenshots-above-fold-${viewport.name}`);

    try {
      console.log(chalk.gray(`    📸 Capturing ${viewport.name} above-the-fold screenshot (${viewport.width}x${viewport.height})`));
      ...
      // Wait for any lazy-loaded content with timeout
      await Promise.race([
        page.waitForTimeout(2000), // Wait time for content to load
        page.waitForLoadState('networkidle', { timeout: 5000 })
      ]).catch(() => {
        // Continue if load state timeout - not critical
        console.log(chalk.yellow(`    ⚠️  Network idle timeout for ${viewport.name} - continuing with screenshot`));
      ...
      // Generate output path using simple canonical method
      const filename = `${pageName}-${viewport.name}-atf.png`;
      const outputPath = this.sessionManager.buildFilePath(sessionId, pageName, 'screenshots', filename);

      // Ensure directory exists with retry logic
      await this.ensureDirectoryWithRetry(outputPath);

      // Capture above-the-fold screenshot (fullPage: false)
      const screenshotPromise = page.screenshot({
        path: outputPath,
      ...
      console.log(chalk.green(`        📄 Above-the-fold screenshot saved: ${outputPath}`));
      ...
      console.log(chalk.red(`    ❌ Failed to capture ${viewport.name} above-the-fold screenshot: ${errorMessage}`));
    }
